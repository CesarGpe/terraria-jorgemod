...
namespace eslamio.Content.Items.Tools
{
	...
	internal class TarzanHookProjectile : ModProjectile
	{
		...
		public override void Load() { // This is called once on mod (re)load when this piece of content is being loaded.
			...
			chainTexture = ModContent.Request<Texture2D>("eslamio/Content/Items/Tools/TarzanHookChain");
		}
		...
		public override bool? GrappleCanLatchOnTo(Player player, int x, int y) {
			...
			{
				if (Main.tile[x, y].HasTile && Main.tileSolid[tile.TileType] && Main.tile[x, y].HasUnactuatedTile)
					Projectile.Kill();
			}
			return false;
		}
