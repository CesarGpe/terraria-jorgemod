...
internal class UILoader : ModSystem
{
    ...
    {
        Environment.SetEnvironmentVariable("JORGEMOD_UIUPDATE", "1");
    }
        ...
        InterfaceScaleType scale)
    {
        ...
        layers.Insert(index, new LegacyGameInterfaceLayer($"{nameof(eslamio)}: " + name,
            delegate
            {
                if (visible)
    ...
    {
        if (Environment.GetEnvironmentVariable("JORGEMOD_UIUPDATE") is "1")
        {
            Unload();
            OnModLoad();
            Environment.SetEnvironmentVariable("JORGEMOD_UIUPDATE", "0");
        }
